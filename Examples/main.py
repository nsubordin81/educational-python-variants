...
if __name__ == "__main__":
    ...
    assert (
        abs(
            ssk("cat", "cart", 4, lbda, accum=True)
            - (3 * lbda ** 2 + lbda ** 4 + lbda ** 5 + 2 * lbda ** 7)
        )
        < 1e-6
    )
    # From paper
    ...
    print(string_kernel(xs, ys, 2, 1.0))
    assert abs(string_kernel(xs, ys, 2, 1.0)[0, 0] - 0.40824829) < 1e-6

    test = (
        "This is a very long string, just to test how fast this implementation "
        "of ssk is. It should look like the computation tooks no time, unless you're"
        " running this in a potato pc"
    )
    print(ssk(test, test, 30, 0.8, accum=True))
